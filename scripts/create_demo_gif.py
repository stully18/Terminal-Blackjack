...
def panel(draw: ImageDraw.ImageDraw, xy: tuple[int, int, int, int], title: str, border: str = CYAN) -> None:
    ...
    draw.text((xy[0] + 16, xy[1] - 11), f" {title} ", font=FONT_SMALL, fill=border)
...
def card(draw: ImageDraw.ImageDraw, x: int, y: int, label: str, red: bool = False, hidden: bool = False) -> None:
    ...
    draw.text((x + 12, y + 24), "??" if hidden else label, font=FONT_BOLD, fill=outline)
...
def base_frame(notice: str, player_cards: list[str], dealer_cards: list[str], pose: str = "idle") -> Image.Image:
    ...
    panel(draw, (92, 126, WIDTH - 92, 308), "Dealer", GREEN)
    dealer(draw, pose)

    panel(draw, (92, 326, 442, 430), "Dealer Hand", CYAN)
    x = 190
    for label in dealer_cards:
        card(draw, x, 344, label, red="♥" in label or "♦" in label, hidden=label == "??")
        x += 76

    panel(draw, (478, 326, WIDTH - 92, 430), "Player", CYAN)
    x = 568
    for label in player_cards:
        card(draw, x, 344, label, red="♥" in label or "♦" in label)
        x += 76

    draw.rounded_rectangle((92, 452, WIDTH - 92, 504), radius=4, fill="#121a22", outline="#334155")
    draw.text((156, 462), "H hit   S stand   D double", font=FONT_BOLD, fill=CYAN)
    draw.text((156, 482), "S win 25% EV -0.49x  |  H win 58% EV +0.23x  |  D win 58% EV +0.46x", font=FONT_SMALL, fill=MUTED)
    return image


def whoosh_frame(position: int) -> Image.Image:
    image = base_frame("Dealer slides a card to you", ["9♠", "2♣"], ["??", "7♠"], "deal")
    draw = ImageDraw.Draw(image)
    y = 290
    start = 210
    end = 720
...
def main() -> None:
    ...
    frames = [
        base_frame("Your move", ["9♠", "2♣"], ["??", "7♠"]),
        base_frame("Your move", ["9♠", "2♣"], ["??", "7♠"]),
        *[whoosh_frame(i) for i in range(6)],
        base_frame("Hand 1 has 21. Standing.", ["9♠", "2♣", "K♥"], ["??", "7♠"]),
        base_frame("Dealer reveals", ["9♠", "2♣", "K♥"], ["4♣", "7♠", "Q♦"]),
        base_frame("Player wins.", ["9♠", "2♣", "K♥"], ["4♣", "7♠", "Q♦"]),
    ]
    frames[0].save(
        OUTPUT,
        save_all=True,
